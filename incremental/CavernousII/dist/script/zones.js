...
let displayZone = 0;
class Zone {
    constructor(name, map, goalReward = null, goal2Reward = null) {
        this.index = 0;
        this.manaDrain = 0;
        this.name = name;
        ...
        this.goal2Reward = goal2Reward;
        this.goal2Complete = false;
        this.map = map.slice();
	...
	this.map = this.map.map(row => row.replace("ɤ", "█"));
	this.map = this.map.map(row => row.replace("ɤ", "█"));	// only works up to two in the same row. If you need more demongates on one row, add a third copy of this rule.
	this.map = this.map.map(row => row.replace("†", "█"));
	}
    ...
    enterZone() {
        ...
        this.startStuff = stuff
            ...
            .map(s => {
            s.resetMin();
            this.manaDrain = this.manaDrain+s.drain
		return {
                name: s.name,
                count: s.count
            };
...
const zones = [
	...
	"███████████████████",
        ...
        getMessage("Hostile Realm").display();
        realms[3].unlock();
    }),
    new Zone("Zone 8", [
        "████████████████",
